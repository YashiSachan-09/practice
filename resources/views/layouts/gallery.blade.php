...
<head>
    ...
    <meta name="description" content="@yield('meta_description', 'Anayra — a curated marketplace for painters, sculptures, NFT editions, and exclusive gallery collaborations.')">

    <title>{{ $title ?? 'Anayra' }}</title>

    <script>
        (() => {
            const stored = localStorage.getItem('anayra-theme');
    ...
    <script>
        (() => {
            ...
            const apply = (next) => {
                ...
                try {
                    localStorage.setItem('anayra-theme', t);
                } catch (e) {}
